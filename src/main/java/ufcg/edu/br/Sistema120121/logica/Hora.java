...
/**
 * Classe hora, responsável por gerenciar horas no sistema.
 * 
 */
public class Hora {
	...
	public Hora(String hora) throws HourErrorException {
		if (horaValida(hora)) {
			...
			throw new HourErrorException("Hora inválida");
	}
	 ...
	 * @return true - caso a hora seja valida. false - caso a hora não seja
	 *         valida.
	 */
	public boolean horaValida(String Hora) {
		...
		if (Hora.length() != 5)
			return false;
		if ((verificaHora(hora)) && (verificaMinutos(minutos))
				&& (Hora.length() == 5))
			return true;
		return false;
	}
	 ...
	 * @param hora
	 *            a hora a ser verificada.
	 * @return true - para formato correto. false - para formato incorreto.
	 */
	...
	public boolean verificaMinutos(String minutos) {
		if ((minutos == null) || (minutos.isEmpty())
				|| (minutos.matches("[^0-9]*"))
				|| (Integer.parseInt(minutos)) < 0
				|| (Integer.parseInt(minutos)) > 59)
			return false;
		return true;
	}
	...
	public String getHoraAtual() {
		...
		String horaAtual = formatador.format(calendario.getTime()).substring(
				13, 18);
		return horaAtual;
	}
	 ...
	 * @return 0 - objetos iguais. >0 - para objeto maior que o objeto passado.
	 *         <0 - para objeto menor que o objeto passado.
	 */
	public boolean comparaHora(Hora hora) {
		if ((Integer.parseInt(this.getHora()) - Integer
				.parseInt(hora.getHora())) > 0)
			return true;
		else {
			if ((Integer.parseInt(this.getMinutos()) - Integer.parseInt(hora
					.getMinutos())) > 0)
				return true;
		}
		return false;
